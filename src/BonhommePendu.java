...
//Jeu de bonhomme pendu créée par Benjamin Kostiuk and Vyvy Ngo

/* 
 * Ce projet utilise le concept des charact�res et des positions de ces charact�res dans l'alphabet
 * On assume que 'a' � la position 0, 'b' = 1, etc. 
 * Plusieurs fois le charact�re est �chang� avec sa position dans l'alphabet
 */ 
public class BonhommePendu extends JFrame implements KeyListener {

	private String mot;		//Mot choisi al�atoirement du fichier texte
	//Contient la URLs des images utilis�es
	...
	private JMenuItem afficherRegles = new JMenuItem("Afficher R�gles");		
	...
	public BonhommePendu(Joueur j) {	//Constructeur de la fen�tre du jeu
		super("BonhommePendu");
		setBackground(Color.WHITE);
		setSize(1200, 700);
		//Ajoute l'image de l'arri�re plan
		try {
		...
		jouerSon(true, ARRIERE_PLAN);	//Joue la musique en arri�re-plan
		joueur = j;						//Donne la valeur au joueur
		activerCle = true;				
		newGame(0);						//Commence un nouveau jeu avec la difficult� facile
		
		menuBar = new JMenuBar();		//Cr�e la barre menu
		menu = new JMenu("Options");
		menuBar.add(menu);
		subMenu = new JMenu("Nouveau Jeu");
		menu.add(subMenu);
		
		ButtonGroup group = new ButtonGroup();	//Cr�e un buttonGroup pour g�rer les radioButtons 
		...
		menu.add(quitter);						//Ajoute les options quitter, afficher r�gles et
		menuRegles = new JMenu("R�gles");		//classement au menu
		...
		setImagePositions();		//Ceci cr�era et placera les positions de images
		
		radioMenuButtonHandler handler = new radioMenuButtonHandler();	//Cr�e un handler pour le radioBoutons
		...
		afficherRegles.addActionListener(new ActionListener() {	//Classe anonyme pour le bouton afficherR�gles

			@Override
			public void actionPerformed(ActionEvent arg0) {
				FenetreRegles frame = new FenetreRegles();		//Cr�e une fen�tre pour afficher les r�gles
		...
		voirClassement.addActionListener(new ActionListener() {		//Classe anonyme pour le classement
			...
			public void actionPerformed(ActionEvent e) {
				...
				FenetreJoueurs.openFile();					//Lit les scores et les ajoute � la liste de joueurs
	...
	private class radioMenuButtonHandler implements ActionListener {	//Classe priv�e pour g�rer les radioBoutons du menu
	...
	public static void openFile() {		//Ouvre le fichier texte qui contient les mots
		try {
			...
			JOptionPane.showMessageDialog(null, "Erreur � ouvrir le fichier", "Erreur", JOptionPane.ERROR_MESSAGE);
			System.exit(1);
		}
	}
	
	public static String readRecords(int difficulte) {			//Lis les mots du fichier et retourne un mot choisi al�atoirement
		ArrayList<String> mots = new ArrayList<String>();
		SecureRandom rdm = new SecureRandom();		
		
		try {
			while(input.hasNextLine()) {		//Ajoute un mot � la liste s'ils appartiennent � la difficulte
				String mot = input.nextLine();
				if(difficulte == 0 && mot.length() > 6) {	//Mots facile ont une longeur sup�rieur � 6 
					mots.add(mot);
				}
				else if(difficulte == 1 && mot.length() <= 6 && mot.length() >= 5) { //Mots de difficult� moyenne ont une longeur entre 6 et 5 
					mots.add(mot);
				}
				else if(difficulte == 2 && mot.length() < 5) { //Mots difficile ont une longeur inf�rieur � 5
					mots.add(mot);
				}
			}
			
			return mots.get(rdm.nextInt(mots.size()));		//Retourne un mot al�atoirement de la liste de mots correspondants
		}
			...
			JOptionPane.showMessageDialog(null, "Erreur : Ne peut pas lire � partir du fichier.", "Erreur", JOptionPane.ERROR_MESSAGE);
		}
		return null;
	}
	...
	public void setImagePositions() {	//Cr�e les objects ImagePlaceholder avec la position abs de chaque image
		int startX = 610;
		...
		//Pour les images des pi�ces du bonhomme
	...
	private boolean containsLetter(int index) { //V�rifie si la lettre pass� est contenu dans le mot
	...
	private char findChar(int x) {	//Retourne le charact�re associ�e avec l'index de la lettre dans l'alphabet
	...
	public int findInt(char a) throws ExceptionLettre {	//Retourne l'index associ�e avec un charact�re
		char x = 'a';
		for(int i = 0; i < 26; i++) {
			if(a == x) {
				return i;
				...
				throw new ExceptionLettre(a);	//Si le charact�re n'est pas contenu dans l'alphabet lance une exception ExceptionLettre
			}
			x++;
		}
		return 'z';
	...
	public void endGame(int hangman) {		//Verifie si la partie est termin� et si le jeu est gagn� ou perdu
		char[] lettres = mot.toCharArray();
		
		if(hangman == 6) {		//Si perdu
			jouerSon(true, GAGNER);	//Jouer le son pour une partie gagn�
			JOptionPane.showMessageDialog(null, "Bonhomme pendu! Jouez encore?", "Jeu perdu", JOptionPane.INFORMATION_MESSAGE);
			activerCle = false;		//D�sactive l'option pour entrer plus de lettres
		}
		else {					
			boolean test = true;
			for(char c : lettres) {		//D�termine si tous les lettres ont �t�s choisies
				try {
			...
			if(test) {	//Si gagn�
				...
				JOptionPane.showMessageDialog(null, "Jeu Gagn�! Jouez encore?", "Jeu gagn�e", JOptionPane.INFORMATION_MESSAGE);
				activerCle = false;
			}
		}
	...
	public void newGame(int difficulte) {		//Initialize un nouveau jeu
		for(int x = 0; x < estChoisi.length; x++) {
			estChoisi[x] = false;		//Remets les lettres choisi � false
		}
		openFile();
	...
	public void paint(Graphics g) {
		super.paint(g);
		Graphics2D g2 = (Graphics2D)g;	//Cr�e un object Graphics 2D
		
		int start = 600 - (90 * mot.length() - 30) / 2;	//Donne la position abs de la premi�re lettre
		...
		for(int x = 0; x < estChoisi.length; x++) {
			if(estChoisi[x]) {
				if(containsLetter(x))	//Si la lettre choisi est contenu dans le mot
				{
					...
					piecesBonhomme++;	//Ajoute un des pi�ces du bonhomme pendu pour chaque mauvaise lettre
				}
			}
		}
		...
		endGame(piecesBonhomme);	//V�rifie si le partie est finie
	}
	...
	@Override
	public void keyTyped(KeyEvent e) {	//M�thode KeyListener 
		if(activerCle) {
			char lettre = e.getKeyChar();
			try {
				estChoisi[findInt(lettre)] = true;	//Mets l'index de la lettre � true 
				jouerSon(containsLetter(findInt(lettre)), LETTRE); 	//Joue le son assosi� avec la lettre
			}
	...
	public void jouerSon(boolean gagner, int choix) {	//M�thode qui choisi quel ficher .wav joueur
		try {
			URL son = null;
	...
	public static void main(String[] args) {		
		FenetreJoueurs login = new FenetreJoueurs();	//Cr�e la fen�tre pour le login du joueur
